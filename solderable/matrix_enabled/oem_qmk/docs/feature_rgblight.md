...
|-------------------|----------|--------------------------------------------------------------------|
|`RGB_TOG`          |          |Toggle RGB lighting on or off 打开或关闭RGB照明                                      |
|`RGB_MODE_FORWARD` |`RGB_MOD` |Cycle through modes, reverse direction when Shift is held   循环切换模式，保持换档时反转方向        |
|`RGB_MODE_REVERSE` |`RGB_RMOD`|Cycle through modes in reverse, forward direction when Shift is held|保持换档时，在倒档和前进方向循环切换模式
|`RGB_HUI`          |          |Increase hue, decrease hue when Shift is held    增加色调，按住Shift时减少色调                   |
|`RGB_HUD`          |          |Decrease hue, increase hue when Shift is held    降低色调，按住Shift时增加色调                   |
|`RGB_SAI`          |          |Increase saturation, decrease saturation when Shift is held  饱和       |
|`RGB_SAD`          |          |Decrease saturation, increase saturation when Shift is held  饱和       |
|`RGB_VAI`          |          |Increase value (brightness), decrease value when Shift is held  亮度    |
|`RGB_VAD`          |          |Decrease value (brightness), increase value when Shift is held  亮度    |
|`RGB_MODE_PLAIN`   |`RGB_M_P `|Static (no animation) mode   静态（无动画）模式                                       |
|`RGB_MODE_BREATHE` |`RGB_M_B` |Breathing animation mode   呼吸动画模式                                         |
|`RGB_MODE_RAINBOW` |`RGB_M_R` |Rainbow animation mode     彩虹动画模式                                         |
|`RGB_MODE_SWIRL`   |`RGB_M_SW`|Swirl animation mode      漩涡动画模式                                          |
|`RGB_MODE_SNAKE`   |`RGB_M_SN`|Snake animation mode      蛇动画模式                                          |
|`RGB_MODE_KNIGHT`  |`RGB_M_K` |"Knight Rider" animation mode   “骑士骑士”动画模式                                    |
|`RGB_MODE_XMAS`    |`RGB_M_X` |Christmas animation mode         圣诞动画模式                                   |
|`RGB_MODE_GRADIENT`|`RGB_M_G` |Static gradient animation mode   静态渐变动画模式                                   |
|`RGB_MODE_RGBTEST` |`RGB_M_T` |Red, Green, Blue test animation mode    红、绿、蓝测试动画模式                            |
