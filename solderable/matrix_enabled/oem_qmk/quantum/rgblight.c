...
void rgblight_buf_setrgb_at(uint8_t r, uint8_t g, uint8_t b, uint8_t index) {
    if (!rgblight_config.enable || index >= RGBLED_NUM) {
        return;
    }
...
void rgblight_task(void) {
    if (rgblight_status.timer_enabled) {
        ...
        uint8_t       delta         =rgblight_config.speed;// rgblight_config.mode - rgblight_status.base_mode;
