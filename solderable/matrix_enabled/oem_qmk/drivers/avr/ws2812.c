...
LED_TYPE ledBuf[DRIVER_LED_TOTAL];
...
uint8_t ligbtLevelSet(uint8_t i)
{
    uint16_t x;
    uint16_t y;
...
void inline ws2812_setleds(LED_TYPE *ledarray, uint16_t number_of_leds) {
    ...
    //ledarray->g=1;
       ...
       // ledBuf[i] = led[i];
        ledBuf[i].r=ligbtLevelSet(ledarray[i].r);
        ledBuf[i].g=ligbtLevelSet(ledarray[i].g);
        ledBuf[i].b=ligbtLevelSet(ledarray[i].b);

    }
    ws2812_setleds_pin(ledBuf, number_of_leds, RGB_DI_PIN);
